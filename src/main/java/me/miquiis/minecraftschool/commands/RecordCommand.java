...
public class RecordCommand {
    public RecordCommand(CommandDispatcher<CommandSource> dispatcher)
    {
        dispatcher.register(Commands.literal("record").then(Commands.literal("start").then(Commands.argument("name", StringArgumentType.string()).executes(context -> {
            ...
            return 1;
        })).then(Commands.literal("play").then(Commands.argument("name", StringArgumentType.string()).executes(context -> {
            return MinecraftSchool.getInstance().getRecordManager().startPlaying(context.getSource().asPlayer(), StringArgumentType.getString(context, "name"));
        })))
